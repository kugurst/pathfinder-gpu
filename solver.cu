...
__device__ nodeList_t *aStar(point_t *grid, human_t *human, int maxWidth, int maxHeight)
{
	node_t *start = (node_t *) malloc(sizeof(node_t));
		...
		start->y = human->posY;
		start->g = 0;
		start->h = (human->posX - human->goalX) * (human->posX - human->goalX) + (human->posY - human->goalY) * (human->posY - human->goalY);
	...
	while (openList->size > 0) {
		...
		for (int yOff = -1; yOff <= 1; yOff++) {
			for (int xOff = -1; xOff <= 1; xOff++) {
				...
				if (xOff + qx >= maxWidth || yOff + qy >= maxHeight || xOff + qx < 0 || yOff + qy < 0)
					continue;
				...
				int hCost = (px - human->goalX) * (px - human->goalX) + (py - human->goalY) * (py - human->goalY);
				int fCost = gCost + hCost;
				debugPrintf("g: %d, h: %d, f: %d\n", gCost, hCost, fCost);
				bool removed = scanList(&openList, px, py, fCost) && scanList(&closedList, px, py, fCost);
...
__device__ bool moveHuman(point_t *grid, human_t *human, nodeList_t *path, int width, int *remainingHumans)
{
	// Get our next point
...
__device__ void transferPath(stat_t *stats, int id, void *results, int width, int height)
{
	debugPrintf("%p, %lu\n", results, sizeof(char));
		...
		int maxWidth, int maxHeight, int numHumans, int *remainingHumans, void *results, unsigned int *itrCnt)
{
	...
	if (id < numHumans) {
		...
		node_t node;
			...
			node.g = 0;
			node.h = (humans[id].posX - humans[id].goalX) * (humans[id].posX - humans[id].goalX) + (humans[id].posY - humans[id].goalY) * (humans[id].posY - humans[id].goalY);
