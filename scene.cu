...
int linearizeGrid(scene_t *scene, point_t **linearGrid)
{
	...
	*linearGrid = (point_t *) malloc(scene->width * scene->height * sizeof(point_t));
...
int buildMap(FILE *mapFile, scene_t *scene, map<string, human_t *> *humanMap, map<string, point_t *> *goalMap)
{
		...
		do {
			...
			if (++curX > max_x) {
				...
				for (int i = 0; i <= max_y; i++)
					grid[i] = (point_t *) realloc(grid[i], (curX + 1) * sizeof(point_t));
			}
